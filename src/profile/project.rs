...
pub enum ProfileSource {
    Installed,
    BuiltinTemplate,
    /// Immutable content generated by an attended concierge plan. The content
    /// itself stays in memory; the redacted plan carries only its fingerprint
    /// and exact route summary.
    Generated,
    Unavailable,
}
...
pub fn project_digest(workgraph_dir: &Path) -> Result<String> {
    // Planning a first-run project profile must remain read-only, so permit an
    // absent final `.wg` component when its repository root already exists.
    // Apply calls this again after init and therefore proves the same physical
    // parent resolves to the same intended graph identity.
    let canonical = match workgraph_dir.canonicalize() {
        Ok(path) => path,
        Err(_) => {
            let parent = workgraph_dir.parent().ok_or_else(|| {
                anyhow::anyhow!("Graph target {} has no parent", workgraph_dir.display())
            })?;
            let parent = parent.canonicalize().with_context(|| {
                format!(
                    "Project profile selection requires an existing canonical repository; could not resolve {}",
                    parent.display()
                )
            })?;
            let name = workgraph_dir.file_name().ok_or_else(|| {
                anyhow::anyhow!(
                    "Graph target {} has no final component",
                    workgraph_dir.display()
                )
            })?;
            if name != ".wg" {
                anyhow::bail!(
                    "A missing first-run graph target must be the repository's .wg directory, not {}",
                    workgraph_dir.display()
                );
            }
            parent.join(name)
...
pub fn plan_project_selection_at(
    ...
    Ok(ProjectSelectionPlan {
        ...
        materializes_global_profile_definition: matches!(
            source,
            ProfileSource::BuiltinTemplate | ProfileSource::Generated
        ),
        project_digest,
...
/// Build a redacted immutable plan for a new generated reusable definition.
/// The caller retains `content` in memory and must pass the same bytes to
/// [`apply_generated_project_selection`]. Existing definitions are never
/// overwritten: choose another generated name if the exact content differs.
pub fn plan_generated_project_selection_at(
    workgraph_dir: &Path,
    name: &str,
    content: &str,
    now: DateTime<Utc>,
    ...
    named::validate_profile_name(name)?;
    let profile_path = named::profile_path(name)?;
    let expected_profile_preimage = file_preimage(&profile_path)?;
    if expected_profile_preimage != "absent" {
        anyhow::bail!(
            "Generated profile '{}' already exists; immutable concierge plans never overwrite reusable definitions",
            name
        );
    }
    let config: Config = toml::from_str(content)
        .with_context(|| format!("Generated profile '{}' is invalid", name))?;
    ...
    Ok(ProjectSelectionPlan {
        ...
        writes_global_config: false,
        writes_global_active_profile: false,
        materializes_global_profile_definition: true,
        project_digest: project_digest(workgraph_dir)?,
        profile: name.to_string(),
        profile_fingerprint: profile_content_fingerprint(content)?,
        profile_source: ProfileSource::Generated,
        selected_at: now.to_rfc3339(),
        expected_association_preimage: file_preimage(&association_path(workgraph_dir))?,
        expected_profile_preimage,
        readiness: readiness_for_config(&config),
    })
}

pub fn plan_generated_project_selection(
    workgraph_dir: &Path,
    name: &str,
    content: &str,
) -> Result<ProjectSelectionPlan> {
    plan_generated_project_selection_at(workgraph_dir, name, content, Utc::now())
}

/// Apply one generated plan atomically across its reusable definition and
/// project association. On association failure, remove only the exact bytes
/// this call created; credentials and unrelated profile/config state are never
/// rollback targets.
pub fn apply_generated_project_selection(
    workgraph_dir: &Path,
    ...
    if plan.version != PROJECT_SELECTION_VERSION
        || plan.scope != "project"
        || plan.profile_source != ProfileSource::Generated
        || !plan.materializes_global_profile_definition
        || plan.writes_global_config
        || plan.writes_global_active_profile
    {
        anyhow::bail!("Invalid generated project profile selection plan");
    }
    if project_digest(workgraph_dir)? != plan.project_digest {
        anyhow::bail!("Project identity changed after planning; nothing was written.");
    }
    if profile_content_fingerprint(content)? != plan.profile_fingerprint {
        anyhow::bail!("Generated profile content changed after planning; nothing was written.");
    }
    let config: Config = toml::from_str(content).context("Generated profile is invalid")?;
    config.validate_model_format()?;
    ...
    if file_preimage(&association_path)? != plan.expected_association_preimage {
        anyhow::bail!("Project profile association changed after planning; nothing was written.");
    }
    let profile_path = named::profile_path(&plan.profile)?;
    if file_preimage(&profile_path)? != plan.expected_profile_preimage
        || plan.expected_profile_preimage != "absent"
    {
        anyhow::bail!("Generated profile target changed after planning; nothing was written.");
    }
    crate::atomic_file::write_atomic_create_new(&profile_path, content.as_bytes())
        .with_context(|| format!("Failed to materialize generated profile '{}'", plan.profile))?;
    let created_preimage = digest_bytes(content.as_bytes());
    ...
    body.push(b'\n');
    if let Err(error) = write_atomic(&association_path, &body) {
        if file_preimage(&profile_path).ok().as_deref() == Some(created_preimage.as_str()) {
            let _ = fs::remove_file(&profile_path);
        }
        return Err(error).context("Failed to write generated project profile association");
    }
    Ok(association)
}
...
pub fn apply_project_selection(
    ...
    match plan.profile_source {
        ...
        ProfileSource::Generated => {
            anyhow::bail!("Generated profile plans require apply_generated_project_selection")
        }
