...
impl HasId for RoleComponent {
    fn entity_id(&self) -> &str {
        &self.id
    }
}
impl HasId for DesiredOutcome {
    fn entity_id(&self) -> &str {
        &self.id
    }
}
impl HasId for TradeoffConfig {
    fn entity_id(&self) -> &str {
        &self.id
    }
}
...
impl HasId for TaskAssignmentRecord {
    fn entity_id(&self) -> &str {
        &self.task_id
    }
}
...
pub fn load_component(path: &Path) -> Result<RoleComponent, AgencyError> {
    load_yaml(path)
}
pub fn save_component(c: &RoleComponent, dir: &Path) -> Result<PathBuf, AgencyError> {
    save_yaml(c, dir, &c.id)
}
pub fn load_all_components(dir: &Path) -> Result<Vec<RoleComponent>, AgencyError> {
    load_all_yaml(dir)
}
pub fn find_component_by_prefix(dir: &Path, prefix: &str) -> Result<RoleComponent, AgencyError> {
    find_by_prefix(dir, prefix, "component")
}

// -- Primitives: DesiredOutcome
pub fn load_outcome(path: &Path) -> Result<DesiredOutcome, AgencyError> {
    load_yaml(path)
}
pub fn save_outcome(o: &DesiredOutcome, dir: &Path) -> Result<PathBuf, AgencyError> {
    save_yaml(o, dir, &o.id)
}
pub fn load_all_outcomes(dir: &Path) -> Result<Vec<DesiredOutcome>, AgencyError> {
    load_all_yaml(dir)
}
pub fn find_outcome_by_prefix(dir: &Path, prefix: &str) -> Result<DesiredOutcome, AgencyError> {
    find_by_prefix(dir, prefix, "outcome")
}

// -- Primitives: TradeoffConfig (formerly Motivation)
pub fn load_tradeoff(path: &Path) -> Result<TradeoffConfig, AgencyError> {
    load_yaml(path)
}
pub fn save_tradeoff(t: &TradeoffConfig, dir: &Path) -> Result<PathBuf, AgencyError> {
    save_yaml(t, dir, &t.id)
}
pub fn load_all_tradeoffs(dir: &Path) -> Result<Vec<TradeoffConfig>, AgencyError> {
    load_all_yaml(dir)
}
pub fn find_tradeoff_by_prefix(dir: &Path, prefix: &str) -> Result<TradeoffConfig, AgencyError> {
    find_by_prefix(dir, prefix, "tradeoff")
}
...
pub fn save_role(role: &Role, dir: &Path) -> Result<PathBuf, AgencyError> {
    save_yaml(role, dir, &role.id)
}
pub fn load_all_roles(dir: &Path) -> Result<Vec<Role>, AgencyError> {
    load_all_yaml(dir)
}
pub fn find_role_by_prefix(dir: &Path, prefix: &str) -> Result<Role, AgencyError> {
    find_by_prefix(dir, prefix, "role")
}
...
pub fn save_agent(agent: &Agent, dir: &Path) -> Result<PathBuf, AgencyError> {
    save_yaml(agent, dir, &agent.id)
}
pub fn load_all_agents(dir: &Path) -> Result<Vec<Agent>, AgencyError> {
    load_all_yaml(dir)
}
pub fn find_agent_by_prefix(dir: &Path, prefix: &str) -> Result<Agent, AgencyError> {
    find_by_prefix(dir, prefix, "agent")
}
...
pub fn load_assignment_record_by_task(
    dir: &Path,
    task_id: &str,
) -> Result<TaskAssignmentRecord, AgencyError> {
    let path = dir.join(format!("{}.yaml", task_id));
    if !path.exists() {
        return Err(AgencyError::NotFound(format!(
            "No assignment record for task '{}'",
            task_id
        )));
    }
    load_yaml(&path)
...
impl LocalStore {
    ...
    pub fn components_dir(&self) -> PathBuf {
        self.path.join("primitives/components")
    }
    pub fn outcomes_dir(&self) -> PathBuf {
        self.path.join("primitives/outcomes")
    }
    pub fn tradeoffs_dir(&self) -> PathBuf {
        self.path.join("primitives/tradeoffs")
    }
    pub fn roles_dir(&self) -> PathBuf {
        self.path.join("cache/roles")
    }
    pub fn agents_dir(&self) -> PathBuf {
        self.path.join("cache/agents")
    }
...
impl AgencyStore for LocalStore {
    ...
    fn load_components(&self) -> Result<Vec<RoleComponent>, AgencyError> {
        load_all_components(&self.components_dir())
    }
    fn save_component(&self, c: &RoleComponent) -> Result<PathBuf, AgencyError> {
        save_component(c, &self.components_dir())
    }
    fn exists_component(&self, id: &str) -> bool {
        self.components_dir().join(format!("{}.yaml", id)).exists()
    }
    fn load_outcomes(&self) -> Result<Vec<DesiredOutcome>, AgencyError> {
        load_all_outcomes(&self.outcomes_dir())
    }
    fn save_outcome(&self, o: &DesiredOutcome) -> Result<PathBuf, AgencyError> {
        save_outcome(o, &self.outcomes_dir())
    }
    fn exists_outcome(&self, id: &str) -> bool {
        self.outcomes_dir().join(format!("{}.yaml", id)).exists()
    }
    fn load_tradeoffs(&self) -> Result<Vec<TradeoffConfig>, AgencyError> {
        load_all_tradeoffs(&self.tradeoffs_dir())
    }
    fn save_tradeoff(&self, t: &TradeoffConfig) -> Result<PathBuf, AgencyError> {
        save_tradeoff(t, &self.tradeoffs_dir())
    }
    fn exists_tradeoff(&self, id: &str) -> bool {
        self.tradeoffs_dir().join(format!("{}.yaml", id)).exists()
    }
    fn load_roles(&self) -> Result<Vec<Role>, AgencyError> {
