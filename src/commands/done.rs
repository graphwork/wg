...
fn run_inner(
    ...
    // --verify is deprecated: .evaluate tasks now drive terminal status.
    // The verify field is kept as inert metadata for back-compat but no
    // longer runs as a gate.  Emit a deprecation notice so operators know.
    if let Some(verify_cmd) = graph.get_task(id).and_then(|t| t.verify.clone()) {
        eprintln!(
            "Note: --verify is deprecated and no longer runs as a gate. \
             Task '{}' has verify='{}' which is retained as metadata. \
             Terminal status is now determined by .evaluate-{} verdict.",
            id, verify_cmd, id,
        );
    }
