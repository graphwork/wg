...
#[cfg(not(unix))]
fn set_private_dir_permissions(path: &Path) -> Result<()> {
    // On Windows restrict via ACLs (chmod 700 equivalent); on other
    // non-Unix targets there is nothing portable to do.
    #[cfg(windows)]
...
#[cfg(not(unix))]
fn set_private_file_permissions(path: &Path) -> Result<()> {
    // On Windows restrict via ACLs (chmod 600 equivalent); on other
    // non-Unix targets there is nothing portable to do.
    #[cfg(windows)]
    restrict_windows_acl(path);
    #[cfg(not(windows))]
