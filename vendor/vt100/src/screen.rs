...
impl Screen {
    fn text(&mut self, c: char) {
        // LOCAL WORKGRAPH PATCH: wrap the buggy upstream body in
        // catch_unwind. The original `text` panics via .unwrap() on
        // certain grid lookups at column boundaries (column-wrap +
        // wide-char edge cases). Catching here drops the offending
        // char silently rather than killing the reader thread of the
        // embedding TUI. Upstream issue unfixed in 0.15.2; 0.16.0 is
        // blocked for us by ratatui 0.29's exact unicode-width pin.
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            self.text_inner(c);
        }));
    }

    fn text_inner(&mut self, c: char) {
        let pos = self.grid().pos();
        let size = self.grid().size();
