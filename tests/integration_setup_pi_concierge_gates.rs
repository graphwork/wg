//! Focused entry point for the concierge Pi-readiness gate scenario.
//!
//! The scenario drives real PTY wizards and fake `pi` fixtures, so it must
//! run through `worksgood::smoke`'s Linux subreaper rather than as an ad-hoc
//! shell script.
...
#[test]
fn setup_pi_concierge_gates_controlled_process_scenario() {
    ...
    let scenario = manifest
        .scenarios
        .iter()
        .find(|scenario| scenario.name == "setup_pi_concierge_gates")
        .expect("concierge Pi gate scenario remains in the grow-only manifest");
