...
if ! wg --dir "$wg_dir" add "Stuck Task" --id stuck-task --timeout 4h >add.log 2>&1; then
    loud_fail "wg add --timeout 4h failed: $(cat add.log)"
fi
...
if ! wg --dir "$wg_dir" edit stuck-task --timeout 90m >edit1.log 2>&1; then
    loud_fail "wg edit --timeout 90m failed: $(cat edit1.log)"
fi
...
if ! wg --dir "$wg_dir" edit stuck-task --verify-timeout 15m >edit2.log 2>&1; then
    loud_fail "wg edit --verify-timeout 15m failed: $(cat edit2.log)"
fi
...
if ! wg --dir "$wg_dir" edit stuck-task --timeout "" --verify-timeout "" >edit3.log 2>&1; then
    loud_fail "wg edit --timeout '' (clear) failed: $(cat edit3.log)"
fi
...
if wg --dir "$wg_dir" edit stuck-task --timeout "not-a-duration" >edit4.log 2>&1; then
