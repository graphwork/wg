...
# Stores a secret in keyring, configures an endpoint with api_key_ref,
# verifies wg endpoints list shows the key as present (✓).
...
# Store a test secret in keyring
SECRET_NAME="smoke-api-key-ref-$$"
SECRET_VALUE="sk-api-ref-test-${RANDOM}"

cleanup_secret() {
    HOME="$SMOKE_HOME" wg secret rm "$SECRET_NAME" 2>/dev/null || true
}
add_cleanup_hook cleanup_secret

HOME="$SMOKE_HOME" wg secret set "$SECRET_NAME" --value "$SECRET_VALUE" 2>&1 \
    | grep -q "stored in keyring backend" \
    || { echo "FAIL: storing test secret"; exit 1; }

# Initialize a minimal workgraph
HOME="$SMOKE_HOME" wg --dir "$SMOKE_DIR" init --name "smoke-test-$$" --yes 2>/dev/null || true
...
ENDPOINTS_OUT=$(HOME="$SMOKE_HOME" wg --dir "$SMOKE_DIR" endpoints list 2>&1)
echo "endpoints output: $ENDPOINTS_OUT"
echo "$ENDPOINTS_OUT" | grep -q "test-endpoint" \
    || { echo "FAIL: endpoint not listed"; exit 1; }
# The ✓ appears on the api_key line — check the whole output
...
HOME="$SMOKE_HOME" wg secret check "keyring:${SECRET_NAME}" 2>&1 | grep -q "is reachable" \
