...
7. **Do not promise `pi-worksgood` as an npm install today.** `npm view pi-worksgood` and `npm view @worksgood/wg-pi-plugin` both returned 404. The shipped, version-locked integration is the plugin embedded in the verified WorksGood binary, currently installed with `wg pi-plugin install`. A future Pi-facing umbrella package is a separate publishing/security decision.
...
| `wg pi-plugin status`; `wg pi-plugin install` twice | Status correctly identified missing console wiring. Install was successful; the second run left `settings.json` byte-identical. Because this was a checkout build it used live `pi-plugin/dist`; a release build uses the embedded cache. |
...
| `pi list --no-approve` | “No packages installed.” The WG plugin is a settings extension, not a Pi package; this is expected but can confuse users. |
...
| Pi plugin Node backend | `pi-plugin/src/wg-backend.ts:118` calls `pi.exec("wg", …)` for every tool; human-console compat check also shells `wg pi-plugin compat-version`. A collision can send `show` to WireGuard and put network configuration/error text into the LLM. | Highest-priority blocker. Backend must receive an absolute receipt-verified WorksGood path or use canonical `worksg`; hermetic WG→Pi supplies it in env. Human-console setup records/resolves it safely. Missing canonical binary must fail loud, not silently skip compat. Re-embed plugin and bump compat if wire contract changes. |
...
| Packages/integrations | Pi extension package remains `@worksgood/wg-pi-plugin`; variables are `WG_*`; graph dir is `.wg`; federation IDs use `wgid:`. | These do not collide with OS command lookup and should remain stable unless separately designed. Command spelling and product/data namespace are different concerns. |
...
The UI may label it “WorksGood for Pi,” but logs and receipts use `wg-pi-plugin`. `pi list` will not show it because it is a settings extension, not a Pi package. Onboarding must explain that distinction.
...
| `${XDG_CACHE_HOME:-~/.cache}/wg/pi-plugin/<compat>/` | WG | Materialized embedded core plugin. | Disposable; remove only transaction-created cache version, and only if no settings reference remains. |
