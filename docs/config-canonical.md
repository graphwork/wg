...
All G-scope, all current. Defaults: `cleanup_verification = true`, `recovery_branch_max_age = 604800` (7d), `recovery_branch_max_count = 10`, `cleanup_job_queue = true`, `cleanup_queue_size = 50`, `recovery_prune_interval = 3600` (1h), and `disk_sentinel_enabled = true`. Predictive admission reserves schema-2 physical private build deltas above one immutable shared baseline; legacy full-tree high-water is not reused. An explicit `false` remains a lint-visible emergency override. Cleanup, ownership accounting, stream bounds, and source/artifact preservation remain independent. Code: `src/config.rs` (`ResourceManagementConfig`).
