...
| **workgraph** | **HTTP API via shell scripts** | `~/.agency-workgraph-token` |
...
### 2.4 `using agency with workgraph.md` (5,874 bytes)

**workgraph is explicitly supported as a first-class integration.** This doc covers:
- Prerequisites: Agency v1.2.2+, `agency serve` running, workgraph token
- Two translator scripts in `translators/workgraph/`
...
- Comparison table: MCP vs CLI vs workgraph integration
...
1. **Dedicated integration doc**: `docs/integrations/using agency with workgraph.md`
2. **Translator scripts**: `translators/workgraph/agency-assign-workgraph` (batch assignment) and `translators/workgraph/agency-wg-executor.sh` (per-task executor)
3. **Dedicated token type**: `~/.agency-workgraph-token` with `client_id: workgraph`
4. **README routing table**: workgraph listed alongside Claude Code and Superpowers as a first-class integration

### How the integration works today

The current integration is **shell-script-based HTTP**, operating outside of workgraph's coordinator:
...
### How the agent/task model relates to workgraph's
...
| Concept | Agency location | workgraph location |
|---------|----------------|-------------------|
...
- workgraph's assignment is LLM-based or forced
- Integration: workgraph could call Agency's assign API to get richer, primitive-backed compositions

**C. Evaluation feedback loop**
- Agency records evaluations against specific primitive compositions, building performance data
- workgraph has its own four-dimensional evaluation + FLIP scoring
- Integration: evaluation results could flow from workgraph → Agency to improve future compositions

**D. Batch pre-flight**
- Agency's batch assignment endpoint (`POST /projects/{project_id}/assign`) is designed for exactly workgraph's pattern: assign all open tasks at once before execution begins
- The existing `agency-assign-workgraph` translator already implements this

**E. Evolution coordination**
- Agency's evolver does random perturbation + LLM variation on compositions
- workgraph's evolver creates/retires roles and tradeoffs based on performance data
- These could run in concert: Agency evolves primitives, workgraph evolves task-role assignments

### 5.3 Gaps / friction points

1. **Duplicated agency systems**: Both repos implement role/tradeoff/agent/evaluation/evolution independently. workgraph's is Rust-native; Agency's is Python with SQLite+embeddings.

2. **Different primitive models**: Agency primitives are text descriptions with embeddings and quality scores. workgraph primitives are structured types with fields like `skills`, `desired_outcome`, `acceptable_tradeoffs`, `non_negotiable_constraints`.

3. **Different evaluation models**: Agency uses a simple score + text evaluation. workgraph uses four-dimensional scoring + FLIP.
...
| POST | `/projects/{id}/assign` | Batch assignment (the key endpoint for workgraph) |
...
│   └── using agency with workgraph.md  # ← First-class WG integration
├── primitives/
...
└── translators/
    ...
    └── workgraph/                      # ← workgraph translators
...
6. **The existing integration is pre-flight, not runtime**: The translator scripts batch-assign before execution starts. There's no feedback loop during execution, and evaluations currently flow one way (workgraph executor → Agency API).
