...
sequenceDiagram
    ...
    Note over WD,K: settled/outbox — src/pi_watchdog/mod.rs:1362-1381,1676-1781; src/commands/pi_watchdog.rs:249-310
...
|---|---|---|---|
| `turn_start` / request-start aliases | `ProviderRequestStarted`; phase and meaningful clock advance | projected | `src/pi_watchdog/mod.rs:998-1006,1299-1302` |
| `message_start` / response-start aliases | `ProviderResponseStarted` | projected | `src/pi_watchdog/mod.rs:1007-1009,1303-1306` |
| `message_update` thinking/text/tool-call deltas | bounded activity plus `ThinkingDelta`/one-token `TokenDelta` | projected without raw text | `src/pi_watchdog/mod.rs:1012-1023,1124-1188,1313-1315` |
| tool start/update/end | tool/effect state and bounded activity | projected; start is conservatively non-idempotent until an end receipt | `src/pi_watchdog/mod.rs:1025-1067,1189-1234,1395-1430` |
| `turn_end` | deduplicated usage totals and `UsageReceipt` | projected only when usage changed | `src/pi_watchdog/mod.rs:1069-1075,1235-1281,1325-1327` |
| `provider_retry` | `ProviderRetry` | projected, but current `auto_retry_start/end` names are not | `src/pi_watchdog/mod.rs:1010,1307` |
| `compaction_retry` | `CompactionRetry` | only this older spelling is projected | `src/pi_watchdog/mod.rs:1011,1308` |
...
| ordinary `agent_end` | bounded `native_activity` only | not settled/final; only `willRetry:true` becomes `AgentEndWillRetry` | `src/pi_watchdog/mod.rs:1076-1080,1126-1130,1310-1312` |
| `agent_settled` | `CompletionHandoff`, `NeedsFinalization`, continuation reservation/marker | semantic-neutral handoff is durable, but returned launch/delivery actions are not executed | `src/pi_watchdog/mod.rs:1079-1081,1362-1381,1676-1717` |
