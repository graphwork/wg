...
> **2026-09-15 repair note (`repair-opaque-pi-launch-contract`):** The implementation gaps found by this report have since been repaired behind the same opt-in flag, without changing this report's historical evidence or authorizing rollout. Opaque authoring now accepts `pi:openai-codex/gpt-5.6-sol` and passes `openai-codex/gpt-5.6-sol` as one Pi model argument; a complete worker/review launch plan is pinned and checked at preflight and launch; legacy colon-to-slash conversion is available only through exact operator-declared `wg migrate opaque-pi-route`; the existing managed RPC adapter accepts the same unsplit selector and owns/reaps its process groups; strict reviewer unavailability parks the candidate as typed `NeedsReview` rather than consuming a source retry. A disposable real-Pi validation using only existing Pi-owned authentication then ran the exact selector through one WG worker plus hermetic FLIP/Eval review, produced the neutral report, recorded three content-addressed one-shot launch attributions, and landed exactly once (`b3:42d9519e6755a7a077513ae35d6f1120062af800f3ff5a463ddb0bf002084395`). The stable path remains unchanged and rollout is still not automatic.
