...
2. **Fix `Config::global_dir()` to mirror `main.rs::resolve_workgraph_dir`.** Currently `~/.workgraph` literal (audit §1 "Stale alert"); should be `~/.wg` first, fall back to `~/.workgraph` for legacy. Reason: silent divergence breaks new users — `wg init --global` writes `~/.wg/config.toml` but `Config::load_global()` reads `~/.wg/config.toml`.
