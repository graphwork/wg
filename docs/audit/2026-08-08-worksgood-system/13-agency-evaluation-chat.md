...
- The initial `integration_chat` run passed 6/9: round-trip storage, instant wakeup, and concurrent messages timed out after 2s with EAGAIN. Two later `env -i` reruns passed 8/9 and 9/9 respectively; only `chat_concurrent_messages` failed in the 8/9 run with the same 2s/EAGAIN timeout. Real daemons plus concurrent audit load make root attribution uncertain, but the concurrency failure reproduced and is intermittent.
...
| `integration_chat` | scrubbed reruns: 8/9 then 9/9 | intermittent `chat_concurrent_messages` 2s IPC EAGAIN timeout; initial less-isolated run was 6/9. |
