...
A #emph[task] is the fundamental unit of work in wg. Every task has an ID, a title, a status, and may carry metadata: estimated hours, required skills, deliverables, inputs, tags. Tasks are the atoms. Everything else---dependencies, scheduling, dispatch---is structure around them.

Tasks are connected by #emph[dependency] edges expressed through the `after` field. If task B lists task A in its `after` list, then B comes after A---it cannot begin until A reaches a #emph[terminal] status, that is, until A is done, failed, or abandoned. This is a deliberate choice: all three terminal statuses unblock dependents, because a failed upstream task should not freeze the entire graph. The downstream task gets dispatched and can decide what to do with a failed predecessor.
...
#figure(
  ...
  caption: [The heartbeat of a wg project.],
) <fig-core-loop>
...
The task graph and the agency are complementary systems with a clean separation. The graph defines _what_ needs to happen and _in what order_. The agency defines _who_ does it and _how they approach it_. Neither depends on the other for basic operation: you can run wg without the agency (every agent is generic), and you can define agency entities without a graph (though they have nothing to do). The power is in the combination.
...
- *Auto-triage.* When a spawned agent dies, the coordinator can automatically triage the outcome using a fast LLM, classifying the result as _done_ (work was complete), _continue_ (inject recovery context and re-dispatch), or _restart_ (begin fresh).
