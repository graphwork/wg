...
*Resolve the context scope.* The coordinator determines how much surrounding context the agent receives by resolving a _context scope_ through a priority chain: the task's own `context_scope` field takes precedence, then the assigned role's default context scope, then the coordinator's configured scope, then the default of `task`. The four levels are cumulative—each tier includes everything from the tier below:
...
wg projects can communicate across repository boundaries through the _peer_ system. `wg peer add <name> <path>` registers another wg instance as a named peer. `wg add "title" --repo <peer-name>` stages a visible draft in the peer graph and prints the explicit peer-directory `wg publish <id> --only` command required to release it.
