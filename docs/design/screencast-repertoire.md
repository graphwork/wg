...
*A collection of screencasts demonstrating whimsical/beautiful CLI projects built entirely with workgraph.*
...
The existing screencasts (haiku, heist, pancakes) and the demo medley catalog focus on demonstrating **workgraph features** — chat decomposition, validation gates, cycles, etc. These are compelling for users who already know they want a task coordination tool.
...
This repertoire flips the emphasis. Each screencast features a **visually striking terminal project** as the main attraction, with workgraph orchestration visible but secondary. The viewer thinks "that's cool, I want to make that" first, and "oh, it was built with workgraph agents" second.

### Dual Goals

1. **Show the output:** A beautiful/whimsical CLI program that gives a "cool" vibe
2. **Show the process:** workgraph agents spawning, coordinating, and building it in parallel
...
#### workgraph Features Highlighted
