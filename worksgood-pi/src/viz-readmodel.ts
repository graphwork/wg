...
import type { VizSnapshot, VizTask, VizTokenUsage } from "./viz-snapshot.js";

export interface VizCounts {
  inProgress: number;
  ready: number;
  blocked: number;
  done: number;
...
/** The selected-task → detail-lines model, bounded-slice edition. */
export interface DetailLines {
  lines: string[];
  /** True when the task log section is present (bounded tail). */
  hasLog: boolean;
}

/** Stable topological-ish order: dependencies before dependents, id as tiebreak. */
export function orderTasks(tasks: VizTask[]): VizTask[] {
  const byId = new Map(tasks.map((t) => [t.id, t]));
  const visited = new Set<string>();
  const ordered: VizTask[] = [];
  const visiting = new Set<string>();
  const visit = (id: string, depth: number) => {
    if (visited.has(id) || depth > tasks.length + 1) return;
    ...
    visiting.add(id);
    for (const dep of [...(task.after ?? [])].sort()) visit(dep, depth + 1);
    visiting.delete(id);
    visited.add(id);
    ordered.push(task);
  };
  for (const t of [...tasks].sort((a, b) => a.id.localeCompare(b.id))) visit(t.id, 0);
  return ordered;
}

function isReady(task: VizTask, statusOf: Map<string, string>): boolean {
...
export function taskCounts(tasks: VizTask[]): VizCounts {
  const statusOf = new Map(tasks.map((t) => [t.id, t.status]));
  const counts: VizCounts = { inProgress: 0, ready: 0, blocked: 0, done: 0, failed: 0 };
  for (const task of tasks) {
    if (ACTIVE_STATUSES.has(task.status)) {
      ...
      if (task.status === "blocked" || !isReady(task, statusOf)) counts.blocked++;
      else counts.ready++;
    }
  }
  return counts;
...
export function widgetLine(counts: VizCounts): string {
  return (
    `wg ▸ ${counts.inProgress} in-progress · ${counts.ready} ready · ` +
    `${counts.blocked} blocked · ${counts.done} done · ${counts.failed} failed`
  );
}

/** Compact human age like the TUI prints (35m / 5h / 12d). */
export function ageOf(iso: string | null | undefined, now: number = Date.now()): string | null {
  if (!iso) return null;
...
export function tokenDisplay(usage: VizTokenUsage | null | undefined): string | null {
  if (!usage) return null;
  const compact = (n: number | undefined): string | null => {
...
export interface TreeLine {
  text: string;
  /** Task id hit-mapped to this rendered line (null for header/blank lines). */
  taskId: string | null;
  depth: number;
}
...
export interface TreeRender {
  lines: TreeLine[];
  /** Root (undependented) count — used for the panel footer. */
  roots: number;
}
...
export function buildTree(
  ...
  const renderNode = (id: string, depth: number, connector: string): void => {
    ...
    if (collapsed) {
      ...
      const markHidden = (hid: string) => {
        if (hiddenByCollapse.has(hid)) return;
        hiddenByCollapse.add(hid);
        for (const k of childrenOf.get(hid) ?? []) markHidden(k);
      };
    ...
    const extras: string[] = [task.status];
    if (tokens) extras.push(tokens);
    if (age) extras.push(age);
  ...
  for (const t of orderTasks(tasks)) {
    if (!visited.has(t.id) && !hiddenByCollapse.has(t.id)) renderNode(t.id, 0, "·");
  }
  return { lines, roots: roots.length };
}

/** Plain lines with the selection marker applied. */
export function treeText(render: TreeRender, selectedId: string | null): string[] {
 ...
 */
export function detailLines(task: VizTask, tasks: VizTask[]): DetailLines {
  const byId = new Map(tasks.map((t) => [t.id, t]));
  const lines: string[] = [];
  lines.push(`── ${task.id} ──`);
  ...
  if (task.assigned) lines.push(`Agent: ${task.assigned}`);
  const age = ageOf(task.started_at ?? task.created_at);
  if (age) lines.push(`Active: ${age}`);
  if (task.retry_count) lines.push(`Retries: ${task.retry_count}`);
  if (task.failure_reason) lines.push(`Failure: ${task.failure_reason}`);
  ...
  if (downstream.length) lines.push(`  before: ${downstream.join(", ")}`);
...
export function lineTaskMap(render: TreeRender): Array<string | null> {
