...
function enabled(env: Record<string, string | undefined>, backend: ContinuationBackend): boolean {
  return Boolean(
    backend.env.taskId &&
...
export function installCompactionContinuation(
  ...
  const locallyQuiescent = (ctx: ExtensionContext) =>
    sawAgentEnd && !agentStartedAfterEnd && openTools.size === 0 && !ctx.isIdle();
